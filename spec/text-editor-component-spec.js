...
describe('TextEditorComponent', () => {
  ...
  describe('overlay decorations', () => {
    ...
    it('stacks overlapping overlays so they do not cover each other', async () => {
      const { component, editor } = buildComponent({
        width: 300,
        height: 180,
        attach: false
      });
      attachFakeWindow(component);

      const markerA = editor.markScreenPosition([4, 2]);
      const markerB = editor.markScreenPosition([4, 2]);

      const overlayElementA = document.createElement('div');
      overlayElementA.style.width = '80px';
      overlayElementA.style.height = '30px';
      overlayElementA.style.margin = '3px';

      const overlayElementB = document.createElement('div');
      overlayElementB.style.width = '80px';
      overlayElementB.style.height = '30px';
      overlayElementB.style.margin = '3px';

      editor.decorateMarker(markerA, {
        type: 'overlay',
        item: overlayElementA,
        class: 'overlay-a'
      });
      editor.decorateMarker(markerB, {
        type: 'overlay',
        item: overlayElementB,
        class: 'overlay-b'
      });

      await component.getNextUpdatePromise();

      const firstOverlayRect = overlayElementA.getBoundingClientRect();
      const secondOverlayRect = overlayElementB.getBoundingClientRect();

      expect(secondOverlayRect.top).toBeGreaterThanOrEqual(firstOverlayRect.bottom);
    });
