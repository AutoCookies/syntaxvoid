...
module.exports =
  class I18n {
    ...
    preload() {
      // Needed because the built SyntaxVoid version attempts to preload packages, and
      // quickly bootstrap the SyntaxVoid window. Meaning it all happens before our
      // initial `this.initialize()` method is called. And before our config file
      // has actually been read.
      // So we need to attempt to load locales for SyntaxVoid without knowing what
      ...
      // The best bet here may be to load all locales, and if SyntaxVoid's locales are
      // loaded when initialize is actually called, we attempt a prune task to remove
      // unused strings and lower memory usage of SyntaxVoid.
      ...
      // if that even works.
      // As for the initial preload of SyntaxVoid locales before we even have our `resourcePath`
      ...
      let tempSyntaxVoidLocalePath = path.resolve(process.resourcesPath, "app.asar", "locales");
      if (!fs.existsSync(tempSyntaxVoidLocalePath)) {
        tempSyntaxVoidLocalePath = path.resolve(__dirname, "..", "locales"); // `yarn start` CLI run
      }
      if (!fs.existsSync(tempSyntaxVoidLocalePath)) {
        tempSyntaxVoidLocalePath = path.resolve(__dirname, "..", "..", "resources", "app.asar", "locales");
      }
      const localesPaths = fs.listSync(tempSyntaxVoidLocalePath, ["cson", "json"]);
    ...
    translateLabel(label) {
      // Since failing to translate menus could crash SyntaxVoid
