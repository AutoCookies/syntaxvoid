...
module.exports = class TextEditorComponent {
  ...
  overlayBoundsForWrapperPosition(decoration, wrapperTop, wrapperLeft) {
    ...
    const top = wrapperTop + marginTop;
    const left = wrapperLeft + marginLeft;
  ...
  overlaysIntersect(boundsA, boundsB) {
    return !(
  ...
  updateOverlayToRender(decoration) {
    ...
    const { screenPosition, avoidOverflow } = decoration;
    const { row, column } = screenPosition;
    let wrapperTop =
    ...
    let bounds = this.overlayBoundsForWrapperPosition(
      decoration,
      wrapperTop,
      wrapperLeft
    );
    ...
    if (avoidOverflow !== false) {
      const flippedElementTop =
        wrapperTop - this.getLineHeight() - bounds.height - bounds.marginBottom;

      if (bounds.bottom > windowInnerHeight && flippedElementTop >= 0) {
        wrapperTop -= bounds.top - flippedElementTop;
        bounds = this.overlayBoundsForWrapperPosition(
          decoration,
          wrapperTop,
          wrapperLeft
        );
      ...
      } else if (bounds.right > windowInnerWidth) {
        wrapperLeft -= bounds.right - windowInnerWidth;
      }
      bounds = this.overlayBoundsForWrapperPosition(
        decoration,
        wrapperTop,
    ...
    decoration.overlayBounds = bounds;
  }
  ...
  updateOverlaysToRender() {
    ...
    for (let i = 0; i < overlayCount; i++) {
      this.updateOverlayToRender(overlays[i]);
    }

    const placedBounds = [];
    for (let i = 0; i < overlayCount; i++) {
      const decoration = overlays[i];
      let { pixelTop, pixelLeft, overlayBounds } = decoration;
      ...
      while (hadCollision) {
        ...
        for (const existingBounds of placedBounds) {
          if (!this.overlaysIntersect(overlayBounds, existingBounds)) continue;

          pixelTop =
            existingBounds.bottom - overlayBounds.marginTop;
          overlayBounds = this.overlayBoundsForWrapperPosition(
            decoration,
            pixelTop,
            pixelLeft
          );
      ...
      decoration.pixelTop = Math.round(pixelTop);
      decoration.overlayBounds = overlayBounds;
      placedBounds.push(overlayBounds);
    }
  }
