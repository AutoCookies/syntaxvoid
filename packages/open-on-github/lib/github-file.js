...
export default class GitHubFile {
  ...
  remoteBranchName () {
    ...
    const localBranch = this.branchName()
    ...
    if (localBranch) {
      return this.encodeSegments(localBranch)
    }

    // Final fallback for detached HEAD states where we can't determine a
    // branch name from local git metadata.
    return 'master'
  }
}
