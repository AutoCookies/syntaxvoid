...
exports.computeCentralityMetrics = computeCentralityMetrics;
exports.getMaxValues = getMaxValues;
/**
 * Build adjacency lists from graph edges
 */
function buildAdjacencyLists(nodes, edges) {
 ...
 */
function computeDownstreamRadius(nodeId, adjacency, depthCap) {
 ...
 */
function computeCentrality(nodeId, adjacency, allNodes, depthCap) {
    ...
    for (let i = 0; i < sampleSize; i++) {
        ...
        const reachableDirectly = new Set();
        const reachableThroughTarget = new Set();
 ...
 */
function bfsWithExclusion(startNode, adjacency, depthCap, excludeNode, visited) {
    const queue = [{ node: startNode, depth: 0 }];
    while (queue.length > 0) {
        const current = queue.shift();
        if (current.node === excludeNode)
            continue;
        if (visited.has(current.node))
            continue;
        if (current.depth >= depthCap)
 ...
 */
async function computeCentralityMetrics(graph, depthCap = 3, chunkSize = 1000, abortSignal) {
 ...
 */
function getMaxValues(centralityMetrics, graph) {
    let maxHub = 0;
    let maxRadius = 0;
    let maxCentrality = 0;
