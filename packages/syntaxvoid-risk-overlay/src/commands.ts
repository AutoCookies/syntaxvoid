...
import { computeRiskDelta, formatRiskDelta } from './storage/deltaTracker';
 ...
 */
export async function cmdRiskSummary(
    riskEngine: RiskEngine,
    historyStore: RiskHistoryStore
): Promise<string> {
    try {
 ...
 */
export async function cmdRiskTop(
    riskEngine: RiskEngine,
    limit: number = 10
): Promise<string> {
    try {
 ...
 */
export async function cmdRiskDelta(
    riskEngine: RiskEngine,
    historyStore: RiskHistoryStore
): Promise<string> {
    try {
        ...
        const delta = computeRiskDelta(prev, current);
        return formatRiskDelta(delta);
    } catch (error) {
        return `❌ Error: ${error instanceof Error ? error.message : 'Unknown error'}`;
    }
}
 ...
 */
export async function cmdRiskPolicy(
 ...
 */
function getRiskIcon(level: "low" | "medium" | "high" | "critical"): string {
    switch (level) {
        case "low": return "✓";
        case "medium": return "⚠";
        case "high": return "⚠";
