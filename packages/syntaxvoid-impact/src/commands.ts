...
export class CommandHandler {
    ...
    async handle(args: string[]): Promise<string> {
        if (args.length < 1) return 'Usage: /sv impact <file> [depth]';

        const command = args[0];
        if (command !== 'impact') return `Unknown command: ${command}`;
        ...
        if (filePathArg) {
            ...
            if (editor) targetFile = editor.getPath();
        }

        if (!targetFile) return 'Could not resolve file. Open a file or specify path.';
