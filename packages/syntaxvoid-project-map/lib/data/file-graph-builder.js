...
class FileGraphBuilder {
    ...
    onDidUpdate(callback) { return this.emitter.on('did-update', callback); }
    onDidStart(callback) { return this.emitter.on('did-start', callback); }
        ...
        try {
            ...
            for (const [filePath, node] of this.nodes) {
                ...
                for (const imp of imports) {
                    ...
                    if (targetPath && this.nodes.has(targetPath)) {
                        ...
                        node.outDegree++;
                        this.nodes.get(targetPath).inDegree++;
                    }
                }
            }

            // 4. Detect Cycles
            const circularEdges = this.cycleDetector.detect(this.edges);
            ...
            this.latestGraph = {
                ...
                stats: {
                    ...
                    circularGroups: 0 // TODO: CycleDetector counting
                }
            };
    ...
    _resolve(sourceFile, importPath) {
        ...
        if (importPath.startsWith('.')) {
            ...
            return this._tryExtensions(path.join(dir, importPath));
        }
    ...
    _tryExtensions(basePath) {
        ...
        // Extensions
        const exts = ['.js', '.ts', '.jsx', '.tsx', '.json', '.vue', '.svelte'];
...
module.exports = FileGraphBuilder;
