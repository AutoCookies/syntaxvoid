...
class FileGraphRenderer {
    ...
    draw(ctx, nodes, edges, hoveredNode, opts = {}) {
        ...
        if (filterText && filterText.trim().length > 0) {
            ...
            if (matchedNodes.size > 0) {
                ...
                for (const edge of edges) {
                    ...
                    if (!s || !t)
                        continue;
                    if (matchedNodes.has(s))
                        relatedNodes.add(t);
                    if (matchedNodes.has(t))
                        relatedNodes.add(s);
                }
            }
        }
        ...
        const isNodeVisible = (n) => {
            if (!isFiltered)
                return true; // everything visible if no filter
        ...
        if (showLinks) {
            ...
            for (const edge of edges) {
                ...
                if (edge.circular) {
                    ...
                    if (isHoverPath)
                        ctx.globalAlpha = 1.0;
                    if (isFiltered && isEdgeVisible(s, t) && !isDimmed)
                        ctx.globalAlpha = 0.7; // Highlight filtered edges
                }
                ctx.stroke();
            }
        }
        ...
        for (const n of nodes) {
            ...
            if (n.isCircular) {
                ...
                if (isMatch)
                    ctx.fillStyle = '#f1c40f'; // Gold for search match
                if (isRelated)
                    ctx.fillStyle = '#2ecc71'; // Green for related
            }
...
exports.default = FileGraphRenderer;
