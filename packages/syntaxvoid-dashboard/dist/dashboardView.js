...
class DashboardView {
    ...
    renderSkeleton() {
        ...
        `;
        // Bind buttons
        (_a = this.element.querySelector('#btn-risk')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', () => {
    ...
    update(data) {
        ...
        if (graphEl && graphEl.parentElement)
            graphEl.parentElement.innerHTML = `
            <h3 class="text-info">Graph</h3>
        ...
        if (riskEl && riskEl.parentElement)
            riskEl.parentElement.innerHTML = `
            <h3 class="text-warning">Global Risk</h3>
        ...
        if (circularEl && circularEl.parentElement)
            circularEl.parentElement.innerHTML = `
        ...
        if (riskList) {
            riskList.innerHTML = risk.topRisks.map((r) => `
                <li>
