...
class InspectorPanel {
    ...
    _onReveal() {
        if (this.currentPath) {
            ...
            if (treeViewPkg && treeViewPkg.mainModule &&
                ...
                treeViewPkg.mainModule.treeView.revealPath(this.currentPath);
            } else if (treeViewPkg && treeViewPkg.mainModule &&
                treeViewPkg.mainModule.createTreeView) {
                treeViewPkg.mainModule.createTreeView().revealPath(this.currentPath);
            } else {
     ...
     * @param {Object|null} hit - { folder, rect } or null
    ...
    _renderDepList(edges, key, limit) {
        ...
        return top.map(edge => {
            ...
            return `
                <li>
                    <span class="dep-name" title="${targetPath}">${name}</span>
                    <span class="dep-count">×${edge.weight || 1}</span>
                </li>
            `;
        }).join('') + (edges.length > limit ? `<li class="text-subtle">+ ${edges.length - limit} more...</li>` : '');
    }
