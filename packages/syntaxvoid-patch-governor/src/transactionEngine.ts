...
export class TransactionEngine {
    ...
    async apply(proposal: PatchProposal): Promise<TransactionRecord> {
        const txnId = uuidv4();
        ...
        try {
            ...
            for (const fileChange of proposal.files) {
                ...
                if (!targetPath) throw new Error(`Could not resolve path: ${fileChange.path}`);
    ...
    async _rollback(record: TransactionRecord, proposal: PatchProposal) {
    ...
    async _cleanup(record: TransactionRecord) {
        for (const staged of record.stagedFiles) {
    ...
    _persistRecord(record: TransactionRecord) {
        const path = this._getRecordPath(record.txnId);
        fs.writeFileSync(path, JSON.stringify(record, null, 2));
    }

    _getRecordPath(txnId: string) {
    ...
    listIncompleteTransactions(): string[] {
        if (!fs.existsSync(this.txnDir)) return [];
        return fs.readdirSync(this.txnDir)
            ...
            .filter(f => {
                try {
                    ...
                    const record = JSON.parse(content) as TransactionRecord;
