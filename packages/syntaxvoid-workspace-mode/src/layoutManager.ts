...
export class LayoutManager {
    ...
    private hideTreeView(attempt = 1) {
        ...
        const treeViewPanel = panels.find((p: Panel) => {
        ...
        if (!found) {
            if (attempt <= 20) { // 10 seconds
                ...
                if (attempt < 3) console.log(`SyntaxVoid: Tree View not found (attempt ${attempt}). Retrying...`);
    ...
    private isTreeViewItem(item: any): boolean {
    ...
    private async ensureProjectMap() {
        // Check if already open
        const exists = atom.workspace.getPaneItems().some((item: any) => item.getURI && item.getURI() === 'syntaxvoid://project-map');
        if (!exists) {
    ...
    private async ensureRiskOverlay() {
        const exists = atom.workspace.getPaneItems().some((item: any) => item.getURI && item.getURI() === 'syntaxvoid://risk-overlay');
        if (!exists) {
