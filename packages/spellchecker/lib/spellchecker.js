...
class Spellchecker {
  constructor() { }
  checkSpellingAsync(corpus, cb) { if (cb) process.nextTick(() => cb(null, [])); return Promise.resolve([]); }
  setDictionary(lang, dictPath) { return true; }
  isMisspelled(word) { return false; }
  checkSpelling(corpus) { return []; }
  add(word) { }
  remove(word) { }
  getCorrectionsForMisspelling(word) { return []; }
  getAvailableDictionaries() { return ['en_US']; }
